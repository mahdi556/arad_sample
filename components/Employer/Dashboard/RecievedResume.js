...
const Ad = ({ item }) => {
  ...
  return (
    <>
          ...
          <div className="d-flex flex-column px-3">
            ...
            <div className="d-flex align-items-center">
              <h5 className={`${style.txt3} `}>
                {" "}
                {item.type == "site" && item.resume.jobCategory[0].fa_name}{" "}
              </h5>
      ...
      <BreakLine />
      {modalVisible && <RequsetInterview visible={modalVisible} />}
    </>
  );
};
  ...
  return (
    <>
      ...
      <div className="bg-white py-5">
        {resumes.length > 0 && status !== "all"
          ? resumes
              .filter((item) => item.status === status)
              .map((item, key) => <Ad item={item} key={item.key} />)
          : resumes.length > 0 &&
            status === "all" &&
            resumes.map((item, key) => <Ad item={item} key={item.key} />)}
      </div>
    </>
  );
};
